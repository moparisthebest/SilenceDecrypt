...
public class MasterCipher {
  ...
  public byte[] decryptBytes(byte[] decodedBody) {
    try {
  ...
  public boolean verifyMacFor(String content, byte[] theirMac) {
    ...
    System.out.println("MasterCipher: Our Mac: " + Arrays.toString(ourMac));
    System.out.println("MasterCipher: Thr Mac: " + Arrays.toString(theirMac));
    return Arrays.equals(ourMac, theirMac);
  }

  public byte[] getMacFor(String content) {
    System.out.println("MasterCipher: Macing: " + content);
    try {
      Mac mac = getMac(masterSecret.getMacKey());
  ...
  private byte[] decodeAndDecryptBytes(String body) {
  ...
  private String encryptAndEncodeBytes(byte[] bytes) {
    ...
    return Base64.getEncoder().encodeToString(encryptedAndMacBody);
  }

  private byte[] verifyMacBody(Mac hmac, byte[] encryptedAndMac) {
    if (encryptedAndMac.length < hmac.getMacLength()) {
      throw new RuntimeException("length(encrypted body + MAC) < length(MAC)");
    }
    ...
    if (!Arrays.equals(remoteMac, localMac))
      throw new RuntimeException("MAC doesen't match.");
